...
function getOpt(name) { const i = args.indexOf(name); return (i >= 0 && i + 1 < args.length) ? args[i+1] : null; }
const cwd = process.cwd();
const inputDir  = getOpt("--input") || path.join(cwd, "input");
const outDir    = getOpt("--out")   || path.join(cwd, "output");
const mergeFile = getOpt("--merge");
const LIST_ONLY = args.includes("--list");
const DEBUG     = args.includes("--debug");

// NEW: table code→classe
...
function tryLengthPrefixed(buf, off) {
  ...
  try {
    ...
    return { name, next: end0 + 1, nameLenOff: off, mode: "len", term, lengthBytes: L }; // NEW lengthBytes
  } catch { return null; }
}
...
function tryNullTerminated(buf, off, maxLen = 40) {
  if (off >= buf.length) return null;
  let end = off, gotTerm = null;
  while (end < buf.length && (end - off) <= maxLen) {
    if (buf[end] === 0x00 || buf[end] === 0x01) { gotTerm = buf[end]; break; }
    end++;
  }
  ...
  try {
    const name = raw.toString("utf8");
    if (!NAME_OK.test(name)) return null;
    return { name, next: end + 1, nameLenOff: off, mode: "nul", term: gotTerm, lengthBytes: (end - off) }; // NEW lengthBytes
  } catch { return null; }
}
...
function parseOneCharacter(buf, headerIdx) {
  ...
  if (cursor < buf.length && buf[cursor] === 0x00) {
    if (cursor + 4 <= buf.length &&
        buf[cursor+1] === 0x08 && buf[cursor+2] === 0x18 && buf[cursor+3] === 0x00) {
      cursor += 4; // on laisse cursor sur le 0x00 initial pour lecture u32 ci-dessous
    }
  } else if (cursor + 4 <= buf.length &&
             buf[cursor] === 0x00 && buf[cursor+1] === 0x08 &&
             buf[cursor+2] === 0x18 && buf[cursor+3] === 0x00) {
    cursor += 4;
  }
  ...
  const className = (classByte != null) ? CLASS_MAP.get(classByte) || null : null;
  const genderName = (genderByte === 0x00) ? "Male" : (genderByte === 0x01) ? "Femelle" : null;

  if (DEBUG) {
    console.error(
      `name='${name}'@${nameLenOff} mode=${mode} delta=${usedDelta} term=${term?.toString(16)} flag=${flag ?? "null"} ` +
      `startOfName=${startOfName} lenBytes=${nameLenBytes} class=0x${classByte!=null?classByte.toString(16).padStart(2,"0"):"??"}(${className??"?"}) ` +
      `gender=${genderByte!=null?genderByte.toString(16):"??"}(${genderName??"?"}) next=${end}`
    );
  }
...
function run() {
  ...
  if (!fs.existsSync(absInput) || !fs.statSync(absInput).isDirectory()) {
    console.error("Le chemin d’entrée n’est pas un dossier valide:", absInput);
    process.exit(1);
  }

  let files = fs.readdirSync(absInput)
    .filter(f => f.toLowerCase().endsWith(".atd"))
    .map(f => path.join(absInput, f))
    .sort();

  if (files.length === 0) {
    console.error("Aucun fichier .atd trouvé dans:", absInput);
    process.exit(2);
  }

  const results = files.map(f => {
    try { return parseAtd(f); }
    catch (e) { if (DEBUG) console.error("Erreur parse", f, e); return { file: path.resolve(f), error: String(e) }; }
  });
    ...
    for (const r of results) {
      ...
      if (r.characters) {
        for (const c of r.characters) {
          ...
          const cls = c.class_name ?? `0x${(c.class_byte??0).toString(16).padStart(2,"0")}`;
          const g   = c.gender ?? (c.gender_byte!=null?`0x${c.gender_byte.toString(16)}`:"?");
